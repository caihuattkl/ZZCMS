...
exports.channelNews = function(req,res,callback) {
		db.query('select * from news where classChildId in(select id from newsclass where classFirstId="'+req.params.id+'")', function(err, rows) {
			if(!err) {
				callback(rows)
			}
		})
...
exports.classNews = function(req,res,callback) {
	var sql ="SELECT classFirstId,classChildId,subTitle,newsUrl,time from news where classChildId in (SELECT id from newsclass WHERE firstId IN (select id from newsclass where id='"+req.params.id+"'))"
		db.query(sql, function(err, rows) {
			if(!err) {
				callback(rows)
			}
