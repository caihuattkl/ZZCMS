...
exports.channelNews = function(req,res,callback) {
		db.query('select * from news where classChildId in(select id from news_class where classFirstId="'+req.params.id+'") ORDER BY time DESC limit 0,11', function(err, rows) {
			if(!err) {
				callback(rows)
			}
		})
...
exports.classAll = function(req,res,callback) {
	var sql ="SELECT id,firstId,subTitle,directoryName from news_class WHERE firstId IN (select id from news_class where id='"+req.params.id+"')"
		db.query(sql, function(err, rows) {
			if(!err) {
				callback(rows)
			}
