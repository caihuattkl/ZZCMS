from apis import router, Depends, Session, create_access_token, token_is_true
...
async def login(form_data: schemas.User, db: Session = Depends(get_db)):
    # 校验不能为空
    if form_data.name is '': return JSONResponses.error("用户名不能为空!")
    if form_data.password is '': return JSONResponses.error("登录密码不能为空!")
    # 连接数据库查询用户信息
    item = crud.get_user(db, form_data.name, form_data.password)
    if item is None: return JSONResponses.error("账号或密码不正确!")
    userInfo = to_dict(item)
    # 成功后,返回token信息
    access_token = await create_access_token(userInfo)
    userInfo["token"] = access_token
    del userInfo["password"]
    return JSONResponses(userInfo, "登陆成功!", 200, 0)


# 查询用户信息
@router.post("/query_user", response_model=[])
async def get_user(body: schemas.LoginUser = Depends(token_is_true)):
    return JSONResponses(body, "获取用户信息成功!", 200, 0)
